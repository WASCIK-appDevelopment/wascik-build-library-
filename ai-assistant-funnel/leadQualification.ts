...
function cleanBusiness(value?: string) {
  ...
  return value
    .replace(/\s+(?:and\s+)?i\s+(?:need|want|would like|am looking for|'m looking for)\b.*$/i, "")
    .trim();
}
...
export function qualifyLead(turns: ConversationTurn[], existing: LeadProfile = {}): LeadQualification {
  ...
  profile.business ||= cleanBusiness(firstMatch(userText, /(?:my business is|company is|business called|i own|i run)\s+([^\n,.!?]{2,100})/i));
  ...
  const questions: Record<string, string> = {
    projectType: "What would you like us to build for you?",
    business: "What kind of business or project is this for?",
    contact: "What email address or phone number should we use for follow-up?",
  };
